...
public class MicrophoneLayer<T extends MicrophoneGhastEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {
	public MicrophoneLayer(RenderLayerParent<T, M> layerParent) {
		super(layerParent);
	}
	...
	public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		if (!stack.isEmpty() && !entitylivingbaseIn.isSinging()) {
			...
			Minecraft.getInstance().getItemInHandRenderer().renderItem(entitylivingbaseIn, stack, ItemTransforms.TransformType.NONE, false, matrixStackIn, bufferIn, packedLightIn);
