...
public class ModelMaracaSpider<T extends MaracaSpiderEntity> extends SegmentedModel<T> implements IHasArm {
    ...
    public void setRotationAngles(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if(entityIn.isAttacking() && entityIn.world.rand.nextFloat() > 0.5F) {
            float randAngle = (float)entityIn.world.rand.nextInt(45);
            this.spiderLeg7.rotateAngleY += randAngle;
            this.spiderLeg8.rotateAngleY += randAngle;
        }
    }
    ...
    public void translateHand(HandSide sideIn, MatrixStack matrixStackIn) {
        this.getLegForSide(sideIn).translateRotate(matrixStackIn);
        matrixStackIn.rotate(Vector3f.XP.rotation(-90.0F));
        matrixStackIn.rotate(Vector3f.YP.rotation(180.0F));
        matrixStackIn.rotate(Vector3f.XP.rotation(90.0F));
